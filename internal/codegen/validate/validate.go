...
func resolveAnyType(dt ast.DataType, f *ast.File) (ast.DataType, error) {
	...
	case *ast.AnyType:
		...
		for _, t := range f.Types {
			if t.Name == v.Name() {
				return &ast.StructType{NamePrv: v.Name(), Pos: v.Pos}, nil
			}
		}
		for _, en := range f.Enums {
			if en.Name == v.Name() {
				return &ast.EnumType{NamePrv: v.Name(), Pos: v.Pos}, nil
			}
		}
		return nil, ast.NewErr(v.Line, "could not resolve unknown type '%s'", v.Name())
	case *ast.MapType:
